  ...
  return function(parameters)
  {
    ...
    return Object.assign(proto, {
      ...
      run: function(results) {
        ...
        function checkUniqueMatches()
        {
          if (results_obj.results.some(function(result) {

            return results_obj.results.some(function(result2) {
              return result !== result2 && JSON.stringify(result.matches[0]) === JSON.stringify(result2.matches[0]);
            });

          })) {
            throw new Error('Non-unique matches not allowed');
          }
        }
        ...
        results_obj = {
          ...
          results: results.map(function(result) {
            ...
            return {
              record: new MarcRecord(result.stepResults[0].record),
              index: result.results.index,
              matches: result.stepResults[1].map(function(record) {
