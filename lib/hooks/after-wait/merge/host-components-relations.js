...
function factory(Promise, Object, shim_array, log, jjv, jjve, protoFactory, schema)
{
...
//		return record_store.read(results_obj.recordStoreHost, 1).then(function(records_store) {
		return new Promise(function(resolveCallback, rejectCallback) {
		    ...
		    function iterate()
		    {
			return reader.read().then(function(results) {
			    if (results.done) {
				resolveCallback(records);
			    } else {
				records.push(results.value);
				return iterate();
			    }
		    ...
		    var records = [],
		    reader = require('marc-record-converters/lib/nodejs').marcxml.createReader(require('fs').createReadStream('foobar.xml'));
		    
		    return iterate();

		}).then(function(records_store) {
		  
		    results_obj.results[0].matches = records_store.slice(0,1);
		    results_obj.results[1].matches = [];
		    results_obj.results[2].matches = records_store.slice(1,2);
