						...
						*/
						function findMissingByIndex(record, index, records)
						{
						    ...
						    if (surrounding_records.length === 1) {
									return getMatchingRecord(Number(new MarcRecord(surrounding_records[0].record.record).get(/^001$/).shift().value) < id ? record_store_ids[0] + 1 : record_store_ids[0] - 1);
						...
						{
							records = records.map(function(record) {
								var record_marc = new MarcRecord(record.record.record),
										matched_record = new MarcRecord(record.matchedRecords[0]);
                
							  return records.filter(function(record2) {
                  return !new MarcRecord(record2.record.record).equalsTo(record_marc);
