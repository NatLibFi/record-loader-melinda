...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    define([
      ...
      '@natlibfi/json-similarity',
      '@natlibfi/marc-record-rank/lib/rank',
      'marc-record-js',
    ...
    module.exports = factory(
      ...
      require('@natlibfi/json-similarity'),
      require('@natlibfi/marc-record-rank/lib/rank'),
...
function factory(Promise, Object, jsonSimilarity, rankFactory, MarcRecord, processorFactory) {
  ...
  return function(parameters)
  {
    ...
    var logger, fn_read_record_store, fn_rank, obj;
    ...
    function findBestMatch(result, record)
    {
      ...
      if (result.length === 0) {
        ...
        logger.debug('Record ' + better.get(/^001$/).shift().value + ' ranks better thank ' + worse.get(/^001$/).shift().value);
        ...
        return obj.findMatchCandidates(record).then(function(found_records) {
          ...
          results.matchedRecords = found_records
            .filter(function(found_record) {
              var result = jsonSimilarity(record.toJsonObject(), found_record, parameters.matching);
