...
function factory(Promise, jsonpath, log, jjv, jjve, MarcRecord, json_path_transformer, objectCompare, match_prototype, schema, json_similarity_schema, json_path_transformations_schema)
{
	...
	function getMatchCandidates(record, query_list, candidates)
	{
		...
		function processTemplate()
		{
			...
			query += query_options.query.slice(offset, results.index);
		...
		log.setLevel('debug');
		log.debug(getQueryStatement());

		return record_store.read(getQueryStatement()).then(function(found_records) {
	...
	proto = match_prototype(parameters.matching);
	...
	proto.findMatchCandidates = function(record)
	{
	    ...
	    record = record instanceof MarcRecord ? record : new MarcRecord(record);
